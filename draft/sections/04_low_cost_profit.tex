...
\smallskip\noindent
\emph{Case 1: $\underline{r}^{*}>\underline{r}>\underline{r}_0$, so $z^{*}>z>v_{\psi}$.}
...
\end{align}
where $v_{\rho}(v)$ is the $c_H$-offer cutoff with posterior belief
$\tilde{\psi}(\beta(v))$ on $(z,z^{*}]$:
\[
  v_{\rho}(v)
  \equiv \frac{c_H-\tilde{\psi}(\beta(v))\,c_L}{1-\tilde{\psi}(\beta(v))}
  = \frac{(1-\psi\rho(\beta(v)))c_H-\psi(1-\rho(\beta(v)))c_L}{1-\psi}.
\]
On $(z,z^{*}]$, $\tilde{\psi}(\beta(v))<\psi$ and $v_{\rho}(v)<v_{\psi}$.
The first line of \eqref{eq:Pi-true-fail} is $\Pi_{\text{fail}}(\underline{r})$
from \eqref{eq:profit-fail-closed}.
The failure term in $\Pi(\underline{r}^{*})$ is \eqref{eq:profit-fail} with
$z^{*}$ in place of $z$; splitting \eqref{eq:profit-fail} at $z$ into
$[v_{\psi},z]$ and $(z,z^{*}]$ and subtracting from \eqref{eq:Pi-true-fail}
cancels the $\int_{z}^{z^{*}} dF(v)^{N}$ terms and gives
\[
  \Pi_{\text{fail}}^{\text{true}} - \Pi_{\text{fail}}(\underline{r}^{*})
  = \delta (N-1) (c_H - c_L)
    \int_{z}^{z^{*}}
      \frac{F(v_{\psi}) - F\bigl(v_{\rho}(v)\bigr)}{F(v)}\,
      dF(v)^{N}
  > 0
\]
for $N\ge 2$, since $v_{\rho}(v)<v_{\psi}$ on $(z,z^{*}]$.
Hence $\Pi_{\text{fail}}^{\text{true}}>\Pi_{\text{fail}}(\underline{r}^{*})$.
...
Then $\mathbf{1}\{\underline{r}>\underline{r}_0\}=0$ and $\Pi_{\text{fail}}=0$ at
$\underline{r}$.
If $\underline{r}^{*}>\underline{r}_0$, the integrals over $(z,z^{*}]$ in
\eqref{eq:Pi-true-fail} are positive and
$\Pi_{\text{fail}}^{\text{true}}>\Pi_{\text{fail}}(\underline{r}^{*})=0$; otherwise
compare success profit only.
